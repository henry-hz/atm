...
%%--------------------------------------------------------------------
deposit(Amount) ->
  gen_fsm:send_event(?SERVER, {deposit, Amount + 1}).
...
%% initialize. We start with the unauthorized state and use the atom nobody
%% to descript a 'null' value, once Erlang don't support null
%%--------------------------------------------------------------------
...
%% called if a timeout occurs. If the account and PIN are valid, we send the response ok back 
%% to the client. The format of the response is {reply, Response, NewStateName, NewStateData}.
%% Following that format, we’ve changed states to authorized and for the data we store the 
%% account name. If the account information was invalid, we respond with the error and reason
%% and stay in the same state with the same state data. Finally, we implement another catch-all 
%% function at the end. You should do this throughout functional programming environments 
%% but it is especially important here, since different states might be receiving actions 
%% meant for other states. For example, what if, for some reason, someone attempts to deposit 
%% while we’re still unauthenticated? We need a catch all to send back an invalid message error.
...
thank_you(_Msg, _From, State) ->
  {reply, {error, thank_you}, unauthorized, State}.
